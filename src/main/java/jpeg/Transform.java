...
public class Transform {
    ...
    {
        return transformOperation(input, type, blockSize, false);
    }

    public static Matrix inverseTransform(Matrix input, TransformType type, int blockSize)
    {
        return transformOperation(input, type, blockSize, true);
    }

    public static Matrix transformOperation (Matrix input, TransformType type, int blockSize, boolean inverse) {
        ...
        Matrix transposedTransformMatrix = transformMatrix.transpose();
        for (int row = 0; row < input.getRowDimension(); row += blockSize) {
            for (int col = 0; col < input.getColumnDimension(); col += blockSize) {
                ...
                if (inverse) {
                    returnMatrix.setMatrix(row,row + blockSize - 1, col, col + blockSize -1,transposedTransformMatrix.times(block).times(transformMatrix));
                } else {
                    returnMatrix.setMatrix(row,row + blockSize - 1, col, col + blockSize -1,transformMatrix.times(block).times(transposedTransformMatrix));
                }
            }
        }
        return returnMatrix;
            ...
            case DCT:
                ...
                for (int i = 0; i < blockSize; i ++) {
                    for (int j = 0; j < blockSize; j ++) {
                        matrix.set(i,j,(Math.sqrt((double) (i == 0 ? 1 : 2) / blockSize) * Math.cos(((2 * j + 1) * i * Math.PI) / (2 * blockSize))));
                    }
                }break;
            case WHT:
                ...
                for (int i = 4; i <= blockSize; i *= 2){
                    ...
                    matrix = transformWHTOperation(matrix, newMatrix, i);
                }
    ...
    public static Matrix transformWHTOperation(Matrix matrix, Matrix newMatrix, int num) {
